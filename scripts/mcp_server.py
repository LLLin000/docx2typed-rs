...
def history_gc(keep_last: int = 50, dry_run: bool = True) -> str:
    ...
    versions. ``commit_sync(label=…)`` pins an intentional user name;
    descriptive labels generated by restore and baseline transitions do not.
    Applied trims are recorded so history_list/history_verify can report
    ``content: trimmed`` and restore/export fail closed. The version commit
    remains in the graph. ``dry_run`` (default) reports without deleting or
    recording a trim."""
    with session.lock:
        workdir = session.require()
...
def commit_sync(operation_id: str | None = None, label: str | None = None) -> CallToolResult:
    ...
    ``label`` names and pins the version beyond the count limit when supplied.
