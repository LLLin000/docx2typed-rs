...
- **Retention policy, v1**: keep the last 50 **trees** plus every explicitly
  pinned version, and report retained/trimming counts. `commit_sync(label=…)`
  marks an intentional user name as `pin=true`; labels generated by restore,
  baseline transition, or table operations are descriptive and do not pin.
...
- Each applied trim is recorded in the small append-only
  `.docx2typed-store/history-trim.jsonl` retention ledger. It is not a second
